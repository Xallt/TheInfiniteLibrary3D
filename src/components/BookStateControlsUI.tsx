...
} from "../components/Bookshelf/Book";
...
function UniformControls({ book, uniformlyOpenedState, onSwitchToReading }: UniformControlsProps) {
  ...
  return (
    <>
      ...
      />
      <button className="panel-btn" onClick={onSwitchToReading}>
        Read Book
      </button>
    </>
  );
...
function ReadingControls({ book, pageSelectedState, onSwitchToUniform }: ReadingControlsProps) {
  ...
  return (
    <div className="page-controls">
      <button className="panel-btn" onClick={handlePrevPage}>
        ←
      </button>
      <button className="panel-btn" onClick={handleNextPage}>
        →
      </button>
      <button className="panel-btn" onClick={onSwitchToUniform}>
        Uniform Open
      </button>
    </div>
  );
...
export function BookStateControlsUI({ book }: BookStateControlsUIProps) {
  ...
  const [bookOpeningState, setBookOpeningState] = useState<BookOpeningState>(
    buildUniformlyOpenedState()
  );
  ...
  return (
    <div className="book-controls-panel panel">
      {bookOpeningState.stateType === "pageSelected" ? (
        <ReadingControls
          book={book}
          pageSelectedState={bookOpeningState as PageSelectedState}
          onSwitchToUniform={onSwitchToUniform}
        />
      ) : (
        <UniformControls
          book={book}
          uniformlyOpenedState={bookOpeningState as UniformlyOpenedState}
          onSwitchToReading={onSwitchToReading}
        />
      )}
    </div>
  );
