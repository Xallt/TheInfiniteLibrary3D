...
export function PDFSelectionModal({
  ...
  const handleCountChange = (index: number, value: number | string | null) => {
    if (activeTab === "url") {
      setUrlResourcesWithCount((prev) => {
        const newResources = [...prev];
        newResources[index] = {
    ...
    if (validResources.length > 0) {
      const expandedResources = validResources.flatMap(({ resource, count }) => {
        if (typeof count === "string") {
          return Array(Math.max(1, Math.min(100, count === "" ? 1 : parseInt(count)))).fill(
            resource
          );
        }
        return Array(Math.max(1, Math.min(100, count))).fill(resource);
      });
  ...
  const getSubmitButtonText = () => {
    ...
    const totalCount = activeResources.reduce((sum, { count }) => {
      if (typeof count === "string") {
        return sum + (count === "" ? 1 : parseInt(count));
      }
      return sum + count;
    }, 0);
    ...
    <div className="count-selector">
      ...
      <input
        ...
        onChange={(e) => {
          ...
          handleCountChange(index, value === "" ? "" : parseInt(value));
        }}
        onBlur={(e) => {
    ...
    <div className="modal-overlay">
      <div className="modal-content">
                ...
                {urlResourcesWithCount.map((resourceWithCount, index) => (
                  <div key={index} className="url-input-row">
                    ...
                    {!singleBookMode && urlResourcesWithCount.length > 1 && (
                      <button className="remove-url" onClick={() => handleRemoveResource(index)}>
                        ×
                      </button>
                    )}
                  </div>
            ...
            <div className="file-tab">
              <div className="file-upload-area">
                ...
                <label htmlFor="pdf-file-input" className="file-input-label">
                  ...
                  <div className="upload-text">Drop PDF files here or click to select</div>
                </label>
              </div>
        ...
        <button
          ...
          disabled={
            (activeTab === "url" ? urlResourcesWithCount : fileResourcesWithCount).length === 0
          }
        >
