...
export class PdfParseResult {
  ...
  constructor(metadata: PDFMetadata, pages: AsyncGenerator<PdfPage>) {
...
export class PdfParser {
  ...
  public async parsePdfToImages(
    ...
    return new PdfParseResult(metadata, this.generatePages(pdf, options));
  }
  ...
  private async *generatePages(
    ...
    for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
      ...
      const blob = await new Promise<Blob>((resolve) =>
        canvas.toBlob((blob) => resolve(blob!), `image/${imageFormat}`, 1.0)
      );
